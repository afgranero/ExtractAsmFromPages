...
def is_address_valid(address):
    # checks if: ...
    # ... address is a valid hexadecimal number followed by H and ...
    # ... addresses always crescent
    if not hasattr(is_address_valid, "prev_address_dec"):
        is_address_valid.prev_address = "-1H"

    address_dec, f_hex_address = hex2dec(address[:-1])
    prev_address_dec, f_hex_prev_address = hex2dec(is_address_valid.prev_address[:-1])
    ...
    return f_hex_address and f_hex_prev_address and (0 <= address_dec <= 65535 and address_dec > prev_address_dec)
